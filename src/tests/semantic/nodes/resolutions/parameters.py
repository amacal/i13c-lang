...
def can_accept_valid_parameter_in_a_snippet():
    ...
    assert resolution.accepted[0].bind == "value"
...
def can_accept_immediate_parameter_in_a_snippet():
    source, resolutions = prepare_resolutions(
        """
            asm main(v@imm: u8[0x01..0x02]) { mov rax, rbx; }
        """
    )
    ...
    assert resolution.accepted[0].id == id
    assert resolution.accepted[0].name == b"v"
    assert resolution.accepted[0].bind == "literal"
    ...
    assert source.extract(resolution.accepted[0].ref) == b"v@imm: u8[0x01..0x02]"
