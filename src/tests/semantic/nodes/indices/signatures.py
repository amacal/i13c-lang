...
def can_index_mixed_signatures():
    source, indices = prepare_indices(
        """
            asm main(x@rax: u8) { }
            fn main(y: u16) { }
    ...
    assert source.extract(acceptance[0].ref) in (b"main(x@rax: u8)", b"main(y: u16)")
    assert source.extract(acceptance[1].ref) in (b"main(x@rax: u8)", b"main(y: u16)")
